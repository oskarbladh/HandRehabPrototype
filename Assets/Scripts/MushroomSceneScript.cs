...
public class MushroomSceneScript : MonoBehaviour
{
  ...
  private void OnTriggerStay(Collider other)
  {
    if (other.gameObject.tag == "Good Basket")
    {
      ...
      elapsedTime = 0;
      //Destroy(this.gameObject);'
      this.gameObject.tag = "Good Basket";
  ...
  {
    GameManager.explorationMode = false;
    if (other.gameObject.tag == "MushroomPlatform")
    {
      StartCoroutine("DestroyMushroom", this.gameObject);
    }
  }

  IEnumerator DestroyMushroom(GameObject mushroom)
  {
    yield return new WaitForSeconds(2.0f);
    GameManager.AllMushrooms.Remove(mushroom);
    GameManager.MushroomsInRange.Remove(mushroom);
    GameManager.explorationMode = true;
    GameManager.objectIsSelected = false;
